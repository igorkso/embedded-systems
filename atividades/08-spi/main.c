...
void update_lcd_display(int ntc, int tempdef) {
    ...
    snprintf(buffer, sizeof(buffer), "NTC:%d Temp:%d", ntc, tempdef);
...
void sd_card_init() {
    ...
    if (spi_bus_initialize(host.slot, &bus_config, SDSPI_DEFAULT_DMA) != ESP_OK) {
        printf("Erro ao iniciar SPI\n");
        return;
    }
    ...
    if (esp_vfs_fat_sdspi_mount(SD_MNT_POINT, &host, &slot_config, &config, &card) != ESP_OK) {
        printf("Erro ao montar SD\n");
        return;
    }
...
void log_temp_to_sdcard(int ntc, int alarm) {
    ...
    if (file) {
        ...
        printf("Falha ao abrir arquivo log.txt\n");
    }
}
